# Given the root of a data directory, generates the corresponding file
# containing the list of all augmented images and their corresponding
# categories.
...
path_prefix = basename(data_dir)

for dataset_dir in listdir(data_dir):
    dataset_prefix = join(path_prefix, dataset_dir)

    image_category_pairs = []
    category_counter = 0
    category_map = {}
    for category_dir in listdir(join(data_dir, dataset_dir)):
        if isdir(join(data_dir, dataset_dir, category_dir)):
            category_map[category_dir] = category_counter
            category_prefix = join(dataset_prefix, category_dir)

            for image in listdir(join(data_dir, dataset_dir, category_dir)):
    ...
    result_file = open(join(data_dir, dataset_dir, name + '.txt'), 'w')
